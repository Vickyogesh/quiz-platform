...
class Backup:
    ...
    def db_dump(self, encrypt=True):
        ...
        command = "mysqldump --routines --triggers -u {db_user} -h {db_host} --port={db_port} --password={db_pass} {db_name} | gzip > {path}"\
            .format(db_user=self.db_user, db_host=self.db_host, db_port=self.db_port,
                    db_pass=self.db_pass, db_name=self.db_name, path=path)
        status = os.system(command)
        if status == 0:
            if encrypt:
